...
namespace Lumen.Services.ProductServices
{
    public class ProductService : IProductService
    {
        ...
        private readonly IProjectEFRepository<OrderItem> orderItemRepository;
        private readonly IProjectEFRepository<Product> productRepository;
        private readonly IProjectEFRepository<Brand> brandRepository;
        public ProductService(IProjectEFRepository<Brand> brandRepository, IProjectEFRepository<OrderItem> orderItemRepository, IProjectEFRepository<ProductCategory> ProductCategoryRepository, IProjectEFRepository<Product> productRepository)
        {
        ...
        public async Task<bool> Delete(long productId)
        {
            try
            {
                ...
                {
                    var dborderItems = orderItemRepository.GetQuery().Where(z => z.ProductId == dbProduct.ProductId).ToList();
                    if (dborderItems.Count > 0)
                    {
                        return false;
                    }
                    else
